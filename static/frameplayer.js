    ...
    var processFrame = function() {
        ...
        if (delta > interval) {
            ...
            if(!player.paused) {
                ...
                var previous_annotation_box = annotations.filter(function( obj ) {
                    return obj.frame == player.currentFrame;
                });
                var previous_tracking_box = tracking_boxes.filter(function( obj ) {
                    return obj.frame == player.currentFrame;
                });
                var params = {
                            previous_frame: player.currentFrame,
                            current_frame: 0,
                            left: 0,
                            top: 0,
                            width: 0,
                ...
                var current_annotation_box = annotations.filter(function( obj ) {
                    return obj.frame == player.currentFrame;
                });
                var tracking_box = tracking_boxes.filter(function( obj ) {
                    return obj.frame == player.currentFrame;
                });

                if (current_annotation_box.length > 0) {
                    update_box(current_annotation_box[0], current_annotation_box[0].text);
                    console.log("Already have something", current_annotation_box[0]);
                }
                else {
                    if (previous_annotation_box.length > 0 || previous_tracking_box.length > 0) {
                        console.log("Have to find a new annotation box");
                        var box = (previous_annotation_box.length > 0) ? previous_annotation_box[0] : previous_tracking_box[0];
                        params.current_frame = player.currentFrame;
                        params.left = box.left;
                        params.top = box.top;
                        params.width = box.width;
                        params.height = box.height;
                        params.new_box = (previous_annotation_box.length > 0) ? 1 : 0
                        $.getJSON($SCRIPT_ROOT + '/_next_tracking', params,
                            function(data) {
                                console.log(tracking_boxes);
                                update_box(data, box.text);
                                tracking_boxes.push({
                                    left: data.left,
                                    top: data.top,
                                    width: data.width,
                    ...
                    else if (tracking_box.length > 0) {
                        update_box(tracking_box[0], tracking_box[0].text);
                        console.log("Already  have something tracking", tracking_box[0]);
                    }
                }
